...
describe('Basic parsing of empty file', function () {
    assert.equal(parserFacade.parseTreeStr(""), "eof_queries")
});

describe('Basic parsing of single define query', function () {
    assert.equal(parserFacade.parseTreeStr("define man sub entity;"), "(eof_queries (query (query_define define (definables (definable (variable_type (type_any (type (label man))) (type_constraint sub (type_any (type (label (type_native entity))))))) ;))) <EOF>)")
});
...
describe('Validation of incomplete define query', function () {
    describe('should have recognize missing operand', function () {
        parseAndCheckErrors("define man;", [
            new parserFacade.Error(1, 1, 10, 11, "mismatched input ';' expecting {'type', 'abstract', SUB_, 'owns', 'regex', 'plays', 'relates', 'when', 'then', 'value'}")
        ]);
    });
});
